...
SERVICE_SET_EFFICIENCY = "set_efficiency"
...
ATTR_MIN_COST = "min_cost"
ATTR_MAX_COMFORT = "max_comfort"
...
SET_EFFICIENCY_SCHEMA = vol.Schema({
    vol.Optional(ATTR_ENTITY_ID): cv.entity_ids,
    vol.Required(ATTR_EFFICIENCY): vol.Coerce(float),
})
...
def set_efficiency(hass, efficiency, entity_id=None):
    ...
    hass.services.call(DOMAIN, SERVICE_EFFICIENCY, data)
...
def async_setup(hass, config):
    ...
    @asyncio.coroutine
    def async_efficiency_set_service(service):
        """Set efficiency on target climate devices."""
        target_climate = component.async_extract_from_service(service)

        efficiency = service.data.get(ATTR_EFFICIENCY)

        for climate in target_climate:
            yield from climate.async_efficiency_set_service()

        yield from _async_update_climate(target_climate)

    hass.services.async_register(
        DOMAIN, SERVICE_SET_EFFICIENCY, async_efficiency_set_service,
        descriptions.get(SERVICE_SET_EFFICIENCY),
        schema=SET_EFFICIENCY_SCHEMA)
...
class ClimateDevice(Entity):
    ...
    def state_attributes(self):
        ...
        if efficiency is not None:
            data[ATTR_EFFICIENCY] = efficiency
            data[ATTR_MIN_COST] = self.min_cost
            data[ATTR_MAX_COMFORT] = self.max_comfort
    ...
    def set_efficiency(self, **kwargs):
        """Change the efficiency setting."""
        raise NotImplementedError()

    def async_set_efficiency(self, **kwargs):
        """Change the efficiency setting.

        This method must be run in the event loop and returns a coroutine.
        """
        return self.hass.loop.run_in_executor(
            None, self.set_efficiency)
    ...
    @property
    def max_comfort(self):
        """Return the maximum comfort."""
        return 10
